...
function insertSpectacles(spectaclesArr) {
    ...
    for (let i = 0; i < visibleSpectacles.length; i++) {
        ...
        cssObject.rotation.z = Math.PI/2; // For installation
...
let mouseX = 0.53; // For installation
let mouseY = 0.5;
...
setInterval(function() {
    ...
    //console.log(Math.abs(cssScene.children[0].position.z / radius - camera.rotation.y));
    for(let i = 0; i < cssScene.children.length; i++) {
        if (Math.abs(cssScene.children[i].position.z / radius - camera.rotation.y) < 0.1) {
            $('.spectacle-item').eq(i).css('background-image', `url('${spectacles[Math.floor(Math.random() * spectacles.length)].image}')`);
            console.log('ayy');
        }
    }
},1000);